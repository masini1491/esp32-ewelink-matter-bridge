...
- Primary constrained target：ESP32-C3 / 4 MB-class，Matter over Wi-Fi；S2C classification：`VIABLE_CONSTRAINED`。
...
## Next evidence-gated stage

- [ ] **Real eWeLink LAN diagnostic / evidence gate**：另行取得明確授權後，建立 CK-BL602 LAN behavior 的 evidence／permission gate。不得從相似裝置推定 UIID、wire schema、encryption applicability、channel numbering 或 convergence semantics；本 queue 不授權 live network、hardware 或真實 credential 操作。
